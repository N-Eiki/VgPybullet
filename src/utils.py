...
def get_option():
    """Getter for input options."""
    parser = argparse.ArgumentParser(
        description='Parameters for traing tossing motions.')

    parser.add_argument(
        '--gripper_type', dest='gripper_type', type=str, default='2f140',
        choices=GRIPPER_LIST, help='set gripper type')
    parser.add_argument(
        '--transport_vel', dest='transport_vel', type=float, default=60.0,
        help='set transporting velocity of the robot arm joints')
    parser.add_argument(
        '--throw_vel', dest='throw_vel', type=float, default=2.620,
        help='et target release velocity of the robot arm')
    parser.add_argument(
        '--obj', dest='obj', default='bottle', type=str,
        choices=OBJECT_INFO.keys(),
        help='set object file name')
    parser.add_argument(
        '--moving_obj', dest='moving_obj', action='store_true', default=False,
        help='use moving object?')
    parser.add_argument(
        '--mode', dest='mode', default=None, type=str,
        help='set display mode')
    parser.add_argument(
        '--grasp_width', dest='grasp_width', type=float, default=0.015,
        help='set value for eef hand closing width for grasping')
    parser.add_argument(
        '--release_width', dest='release_width', type=float, default=0.050,
        help='set value for eef hand opennimg width for releasing')
    parser.add_argument(
        '--reward_func', dest='reward_func', type=str,
        default='success-contact',
        choices=REWARD_LIST, help='set reward function type')
    parser.add_argument(
        '--renders', dest='renders', action='store_true', default=False,
        help='rendering for simulation?')
    parser.add_argument(
        '--sim_view', dest='sim_view', default='back', type=str,
        choices=VIEWPOINTS_INFO.keys(),
        help='set view point for simulation')
    parser.add_argument(
        '--urdf_dir', dest='urdf_dir', action='store',
        default=osp.join(osp.dirname(__file__), '../model'),
        help='directory containing urdfs')
    parser.add_argument(
        '--camera_cfg', dest='cam_cfg', type=str, default='rs-d415',
        choices=CAMERA_LIST, help='set camera configuration name')
    parser.add_argument(
        '--save_snapshots', dest='save_snapshots',
        action='store_true', default=False,
        help='save image snapshots of pybullet window?')
    parser.add_argument(
        '--frame', dest='frame', default='450mm', type=str,
        choices=FRAME_INFO.keys(),
        help='set frame for simulation')
    parser.add_argument(
        '--box_dist', dest='box_dist', type=float, default=-0.52,
        help='set distance from robot to frame')
    parser.add_argument(
        '--measure_time', dest='measure_time',
        action='store_true', default=False,
        help='measure time?')
    parser.add_argument(
        '--weightpath', dest='weightpath', default=None, type=str,
    ...
    parser.add_argument('--training_target', default="static_grasp", choices=["static_grasp", "dynamic_module","dynamic_regulate", "dynamic_grasp", 'dynamic_module'])
    parser.add_argument('--input_frames', default=1, type=int)
    # ------------- Ablation study options -------------
    parser.add_argument('--no_regulate_residual', dest="no_regulate_residual", action='store_true', default=False)
    parser.add_argument('--teacher_student', dest="teacher_student", action='store_true', default=False)
    ...
    parser.add_argument('--load_static_snapshot', dest='load_static_snapshot', action='store_true', default=False,                      help='load pre-trained snapshot of model?')
    parser.add_argument('--static_snapshot_file', dest='static_snapshot_file', action='store')
...
def gen_toss_params(
        ...
        open_len_fin_min,
        vel_min):
    """Generates tossing parameters from action index."""
    open_len_fin = action % open_len_fin_pmax
    target_vel = action % vel_pmax
    return (open_len_fin+open_len_fin_min)*0.001, (target_vel+vel_min)*0.01


def gen_random_observation(
        grasp_posi_height_limit,
        grasp_posi_slide_limit,
        grasp_orient_limit,
        grasp_width_limit):
    """Returns rondomly generated observations."""
    return np.array([
        np.random.uniform(
            grasp_posi_height_limit[0], grasp_posi_height_limit[1]),
        np.random.uniform(
            grasp_posi_slide_limit[0], grasp_posi_slide_limit[1]),
        np.random.uniform(
            grasp_orient_limit[0], grasp_orient_limit[1]),
        np.random.uniform(
            grasp_width_limit[0], grasp_width_limit[1])],
        dtype=np.float64)


def get_reward(throw_results, reward_func_type):
    """Calculates reward based on the action result."""
    reward = 0.0
    if reward_func_type == "success":
        if throw_results['land_success']:
            reward = 4.0
        else:
            if not throw_results['release_success']:
                reward = -1.0
    elif reward_func_type == "success-contact":
        if throw_results['land_success']:
            if throw_results['type'] == "direct":
                reward = 4.0
            elif throw_results['type'] == "wall_contact":
                reward = 2.0
        else:
            if throw_results['release_success']:
                if throw_results['type'] == "wall_contact":
                    reward = 0.5
            else:
                reward = -1.0
    elif reward_func_type == "success-rotation":
        if throw_results['land_success']:
            reward = 4.0
            if throw_results['rot_deg'] > 360:
                reward -= 1.0
            elif throw_results['rot_deg'] > 270:
                reward -= 0.5
        else:
            if not throw_results['release_success']:
                reward = -1.0
    elif reward_func_type == "success-contact-rotation":
        if throw_results['land_success']:
            if throw_results['type'] == "direct":
                reward = 4.0
            elif throw_results['type'] == "wall_contact":
...
def plot_rewards_transition(reward, avg_interval=5, pausesec=0.2):
    """Plots total rewards transition."""
    plt.figure(2)
    plt.clf()
    plt.title('Training...')
    plt.xlabel('Episode')
    ...
    # take avg_interval episode averages and plot them too
    if len(reward) >= avg_interval:
        means = np.array(
            [np.mean(reward[i:i+avg_interval])
             for i in range(len(reward)-avg_interval+1)])
        means = np.hstack((np.zeros(avg_interval-1), means))
        plt.plot(means)

    plt.pause(pausesec)  # pause a bit so that plots are updated
