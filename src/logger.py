...
class Logger():
    ...
    def __init__(self, continue_logging, logging_directory, training_target, exp_keyword="", save_base=None):
        ...
        if self.continue_logging:
            ...
            self.base_directory = os.path.join(logging_directory, timestamp_value.strftime(f'{exp_keyword}{training_target}-%Y-%m-%d.%H:%M:%S'))
        ...
        self.loss_directory = os.path.join(self.base_directory, 'loss')
        self.success_rate_dir = os.path.join(self.base_directory, "success_rate")
        ...
        if not os.path.exists(self.loss_directory):
            os.makedirs(self.loss_directory)
        if not os.path.exists(self.success_rate_dir):
            os.makedirs(self.success_rate_dir)


    def load_logs(self):
        self.success_rate = np.load((os.path.join(self.success_rate_dir, 'success_rate.npy')), allow_pickle=True).item()
        try:
            self.loss_logger = np.load(os.path.join(self.loss_directory, 'loss.npy'), allow_pickle=True).item()
        except:
            self.loss_logger = []
    ...
    def write_to_log(self, log_name, log):
        ...
        np.save(os.path.join(self.transitions_directory, '%s.log.npy' % log_name), log)
    ...
    def visualize_loss_transition(self):

        for key, val in self.loss_logger.items():
            plt.title(f'key loss transition')
            plt.xlabel('Number of Training Steps')
            plt.ylabel('training loss')
            plt.plot(np.arange(len(val)), val, label=f'{key}')
            
        plt.legend()
        plt.savefig(os.path.join(self.loss_directory, 'loss.png'))
        
        plt.clf()

    def save_loss_transition(self):
        np.save(os.path.join(self.loss_directory, 'loss.npy'), self.loss_logger)

    def visualize_success_rate(self):
        # for key in list(success_rate.keys())[::-1]:
        #     val = success_rate[key][:10000]
        #     if 'data'==key:continue
        #     if "last10"==key:continue
        #     plt.title('grasp_success_rate')
        #     plt.xlabel('Number of Training Steps')
        #     plt.ylabel('grasp success rate')
        #     plt.plot(np.arange(len(val)), val, label=f'{key}')
        # plt.legend()
        # plt.savefig(os.path.join(self.success_rate_dir, 'success_rate.png'))
        # plt.clf()

        for key in list(self.success_rate.keys())[::-1]:
            val = self.success_rate[key]
            if key=="last10":continue
            if 'data'==key:continue
            plt.title('grasp_success_rate')
            plt.xlabel('Number of Training Steps')
            plt.ylabel('grasp success rate')
        ...
        plt.legend()
        plt.savefig(os.path.join(self.success_rate_dir, 'success_rate.png'))
        np.save(os.path.join(self.success_rate_dir, 'success_rate.npy'), self.success_rate)
        plt.clf()
